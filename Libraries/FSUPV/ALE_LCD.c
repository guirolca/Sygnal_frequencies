...
TM_LCD_Result_t ALE_LCD_Putc(char c) {
	uint32_t i, b, j,cc;
	...
	if (c != '\n') {
		...
		for (i = 0; i < LCD.CurrentFont->FontHeight; i++) {
			//b = LCD.CurrentFont->data[(c - 32) * LCD.CurrentFont->FontHeight + i];
			for (j = 0; j < LCD.CurrentFont->FontWidth; j++) {
				cc = (j/16)+(i+(c-32)*LCD.CurrentFont->FontHeight)*(LCD.CurrentFont->FontWidth/16);
				b = LCD.CurrentFont->data[cc];
				if ((b << j%16) & 0x8000) {
					TM_DMA2DGRAPHIC_DrawPixel(LCD.CurrentX + j, (LCD.CurrentY + i), LCD.ForegroundColor);
				} else {
					TM_DMA2DGRAPHIC_DrawPixel(LCD.CurrentX + j, (LCD.CurrentY + i), LCD.BackgroundColor);
...
TM_LCD_Result_t ALE_LCD_Puts(char* str) {
	/* Send till string ends or error returned */
	while (*str) {
		/* Check if string OK */
		if (ALE_LCD_Putc(*str) != TM_LCD_Result_Ok) {
			/* Return error */
			return TM_LCD_Result_Error;
		}
...
TM_LCD_Result_t ALE_LCD_Puts_du(char* str) {
	/* Send till string ends or error returned */
	while (*str) {
		/* Check if string OK */
		if (ALE_LCD_Putc_du(*str) != TM_LCD_Result_Ok) {
			/* Return error */
			return TM_LCD_Result_Error;
		}
...
TM_LCD_Result_t ALE_LCD_Putc_du(char c) {
	uint32_t i, b, j;
	
	/* Check current coordinates */
	if ((int16_t)(LCD.CurrentY - LCD.CurrentFont->FontHeight) <= 0 || c == '\n') {
		/* If at the end of a line of display, go to new line and set x to 0 position */
		LCD.CurrentX += LCD.CurrentFont->FontHeight;
		LCD.CurrentY = LCD.StartX;
		
		/* Check X */
		if ((int16_t)(LCD.CurrentY - LCD.CurrentFont->FontHeight) <= 0) {
			LCD.CurrentY = LCD.CurrentHeight;
		}
	...
	if (c != '\n') {
		...
		for (i = 0; i < LCD.CurrentFont->FontHeight; i++) {
			...
			for (j = 0; j < LCD.CurrentFont->FontWidth; j++) {
				if ((b << j) & 0x8000) {
					TM_DMA2DGRAPHIC_DrawPixel(LCD.CurrentX + i, (LCD.CurrentY - j), LCD.ForegroundColor);
				} else {
					TM_DMA2DGRAPHIC_DrawPixel(LCD.CurrentX + i, (LCD.CurrentY - j), LCD.BackgroundColor);
				}
			}
		}
	
		/* Set new current X location */
		LCD.CurrentY -= LCD.CurrentFont->FontWidth;
	}
